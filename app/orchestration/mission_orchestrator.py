...
class MissionOrchestrator:
    ...
    def _read_apriltag_blocking(self, timeout_sec: float | None = None) -> tuple[AprilTagInfo | None, dict | None]:
        deadline = None if timeout_sec is None else (time.monotonic() + max(0.1, float(timeout_sec)))
        while not self._stop_event.is_set():
            ...
            if frame is None:
                time.sleep(0.05)
                continue
            dets, det_anno = self.detector.detect_all(frame, ("facewoodenbox",), max_per_class=3, max_total=3)
            face_candidates = [d for d in dets if d.class_name.strip().lower() == "facewoodenbox"]
            found, anno, metrics = self.apriltag.detect_and_annotate_with_metrics(det_anno)
            self._set_front_frame(anno)
            if found is not None:
                tag_x = None if metrics is None else metrics.get("tag_center_x_px")
                tag_y = None if metrics is None else metrics.get("tag_center_y_px")
                near_face = False
                if tag_x is not None and tag_y is not None:
                    expand = float(config.VISION.apriltag_face_box_expand_scale)
                    near_face = any(
                        self._point_in_expanded_box(float(tag_x), float(tag_y), det, expand)
                        for det in face_candidates
                    )
                if not near_face:
                    self.status_text = "AprilTag seen but rejected (not near FaceWoodenbox)"
                    time.sleep(0.05)
                    continue
                center_err = 0.0 if metrics is None else float(metrics.get("tag_center_err_norm", 0.0))
                yaw_deg = None if metrics is None else metrics.get("tag_pose_yaw_deg")
                center_ok = abs(center_err) <= float(config.VISION.apriltag_accept_center_tol_norm)
                yaw_ok = True if yaw_deg is None else abs(float(yaw_deg)) <= float(config.VISION.apriltag_accept_yaw_deg)
                if not (center_ok and yaw_ok):
                    self.status_text = (
                        "AprilTag seen but rejected "
                        f"(center={center_err:+.3f}, yaw={yaw_deg if yaw_deg is not None else 'na'})"
                    )
                    time.sleep(0.05)
                    continue
    ...
    def _rotate_to_heading(self, target_heading_deg: float, label: str) -> bool:
        deadline = time.monotonic() + max(0.5, float(config.VISION.apriltag_triangle_turn_timeout_sec))
        hold = 0
        hold_needed = 3
        ...
        while not self._stop_event.is_set() and time.monotonic() < deadline:
            ...
            if abs(err_deg) <= float(config.IMU.heading_tolerance_deg):
                hold += 1
            else:
                hold = 0
    ...
    def _run_apriltag_triangle_align(self, metrics: dict | None) -> bool:
        ...
        stand_off_cm = max(0.0, float(config.VISION.apriltag_triangle_standoff_cm))
        advance_z_cm = max(0.0, z_cm - stand_off_cm)
        move1_cm = math.hypot(x_cm, advance_z_cm)
        turn1_deg = math.degrees(math.atan2(x_cm, max(1e-6, advance_z_cm))) if move1_cm > 0.0 else 0.0
        ...
        turn2_mag = abs(float(config.VISION.apriltag_triangle_turn2_deg))
        turn2_deg = -turn2_mag if turn1_deg >= 0.0 else turn2_mag
        logger.info(
            "Triangle plan: x=%.1fcm z=%.1fcm stand_off=%.1fcm turn1=%.1fdeg move1=%.1fcm turn2=%.1fdeg heading=%.1f yaw=%s",
            x_cm,
            z_cm,
            stand_off_cm,
            turn1_deg,
        ...
        self.status_text = f"Triangle plan: turn1={turn1_deg:.1f} move1={move1_cm:.1f} turn2={turn2_deg:.1f}"
        ...
        while not self._stop_event.is_set():
            ...
            if self.state == MissionState.SEARCHING_TARGET:
                ...
                if det_front is not None:
                    ...
                    if self.april_info is None and not self._preapproach_tag_attempted:
            ...
            if self.state == MissionState.READING_APRILTAG:
                ...
                if self.april_info is None:
                    self.status_text = "Scanning AprilTag 52h13"
                    found, metrics = self._read_apriltag_blocking(
                        2.5 if not self._preapproach_tag_attempted else None
                    )
                    self._preapproach_tag_attempted = True
                    if found is not None:
                    ...
                    if self.april_info is None:
                        self.state = MissionState.APPROACHING_TARGET
                        self.status_text = "No triangle: AprilTag not ready in early scan, continue PID approach"
                        logger.info("No triangle on early pass: AprilTag not ready, fallback to PID approach")
                        continue

                if self.april_info is not None:
                    triangle_or_coarse_ok = self._run_apriltag_triangle_align(tag_metrics)
                    if not triangle_or_coarse_ok and bool(config.VISION.apriltag_coarse_align_enabled):
                        logger.warning("Triangle align failed/skipped: %s", self.status_text)
                        triangle_or_coarse_ok = self._run_apriltag_coarse_align()
                        if not triangle_or_coarse_ok:
                            logger.warning("AprilTag coarse align failed/skipped: %s", self.status_text)
                    elif triangle_or_coarse_ok and tag_metrics is not None:
                ...
                if triangle_or_coarse_ok:
                    self.state = MissionState.ALIGNING_ENTRY
                    self.status_text = "Triangle/coarse complete, begin entry align"
                else:
                    fail_reason = self.status_text
                    self.state = MissionState.APPROACHING_TARGET
                    self.status_text = f"Triangle/coarse failed, continue PID approach ({fail_reason})"
                continue
            ...
            if self.state == MissionState.MEASURING_REAR_SIZE:
                ...
                front_raw_now = self.cameras.get_front_frame()
                rear_raw_now = self.cameras.get_rear_frame()
                front_has_box = False
                rear_has_box = False
