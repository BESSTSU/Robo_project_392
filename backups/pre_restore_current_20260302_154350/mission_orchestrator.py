...
class MissionOrchestrator:
    ...
    def _measure_rear_size_once(self) -> bool:
        self.drive.stop()
    ...
    def _run_fsm(self) -> None:
        ...
        while not self._stop_event.is_set():
            ...
            if self.state == MissionState.ALIGNING_ENTRY:
                ...
                require_both = bool(config.VISION.entry_align_require_both)
                if require_both and (det_front is None or det_rear is None):
                    self.drive.stop()
                    self._reset_stuck_watch()
                    self._align_hold_count = 0
                    if self._update_no_box_streak(front_has_box, rear_has_box):
                        ...
                        continue
                    missing = []
                    if det_front is None:
                        missing.append("front")
                    if det_rear is None:
                        missing.append("rear")
                    miss_txt = "/".join(missing) if missing else "unknown"
                    self.status_text = (
                        f"Align entry waiting both cameras ({miss_txt} missing) "
                        f"no-box {self._no_box_streak}/{config.RUNTIME.no_box_stop_frames}"
                    )
                    time.sleep(config.RUNTIME.vision_loop_sec)
                    continue

                if require_both:
                    active_source, active_det, active_frame = "front", det_front, front_raw
                else:
                    active_source, active_det, active_frame = self._select_active_target(
                        det_front, det_rear, front_raw, rear_raw
                    )
                ...
                if active_det is None:
                    ...
                    if self._update_no_box_streak(front_has_box, rear_has_box):
                        ...
                        continue
                    self.state = MissionState.SEARCHING_TARGET
                    self.status_text = "Align lost target, back to search"
                    search_start = time.monotonic()
                    continue
            ...
            if self.state == MissionState.PLANTING:
                ...
                if self._measure_interval_cm > 0:
                    dist0 = self.drive.get_telemetry().distance_cm
                    self._distance_goal_cm = dist0 + self._measure_interval_cm
                    self.status_text = f"Move to rear-measure point by DE={self._measure_interval_cm:.1f} cm"
                else:
